...
<body class="bg-gray-50 text-gray-900 font-sans transition-colors duration-200">
        ...
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-2">
            ...
            <div class="flex items-center space-x-4">
                <div class="flex items-center space-x-1 border-r border-brand-dark-deep pr-4">
                    ...
                    <button onclick="changeFontSize(-1)" class="bg-brand-dark-deep hover:bg-brand-red w-6 h-6 rounded flex items-center justify-center font-bold" title="छोटा आकार">A-</button>
                    <button onclick="changeFontSize(0)" class="bg-brand-dark-deep hover:bg-brand-red w-6 h-6 rounded flex items-center justify-center font-bold" title="सामान्य आकार">A</button>
                    <button onclick="changeFontSize(1)" class="bg-brand-dark-deep hover:bg-brand-red w-6 h-6 rounded flex items-center justify-center font-bold" title="बड़ा आकार">A+</button>
                </div>
                <div class="flex items-center space-x-2">
                    <button onclick="toggleContrast()" class="bg-brand-dark-deep text-[11px] px-2.5 py-1 rounded hover:bg-brand-red font-semibold flex items-center space-x-1" title="कंट्रास्ट बदलें">
    ...
    <header class="bg-white py-4 px-4 shadow-sm border-b border-gray-100">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <a href="{{ route('news.index') }}" class="flex items-center space-x-4">
                ...
                <div class="w-16 h-16 shrink-0 flex items-center justify-center bg-brand-red rounded-2xl shadow-md p-1 group hover:scale-105 transition-transform duration-200" title="छत्तीसगढ़ न्यूज़ एक्सप्रेस">
            ...
            <div class="w-full md:w-auto flex items-center space-x-2">
                <form action="{{ route('news.search') }}" method="GET" class="relative w-full md:w-72">
                    <input 
                        type="text" 
                        name="query" 
                        placeholder="खबरें खोजें (यहाँ टाइप करें)..." 
                        value="{{ request('query') }}"
        ...
        <div class="relative w-full overflow-hidden flex items-center h-full">
            <div class="animate-ticker absolute whitespace-nowrap flex space-x-12 py-1 items-center">
                @foreach($breakingNews as $item)
                    <a href="{{ route('news.show', $item->slug) }}" class="hover:text-brand-accent font-medium flex items-center space-x-2">
    ...
    <nav class="bg-brand-dark text-white sticky top-0 z-30 shadow-md">
        <div class="max-w-7xl mx-auto flex justify-between items-center px-4 overflow-x-auto scrollbar-none md:overflow-x-visible">
            <div class="flex space-x-1 items-center py-1 md:py-0 whitespace-nowrap">
                <a href="{{ route('news.index') }}" class="px-4 py-3.5 text-sm font-semibold hover:bg-brand-red transition-all duration-150 flex items-center space-x-1 {{ request()->routeIs('news.index') ? 'bg-brand-red text-white border-b-2 border-white' : '' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @if(isset($navCategories))
                    @foreach($navCategories as $cat)
                        <a href="{{ route('news.category', $cat->slug) }}" class="px-4 py-3.5 text-sm font-semibold hover:bg-brand-red transition-all duration-150 {{ isset($category) && $category->id == $cat->id ? 'bg-brand-red text-white border-b-2 border-white' : '' }}">
                            {{ $cat->name }}
                        </a>
                    @endforeach
                @endif
                ...
                <div class="relative group whitespace-nowrap">
                    <button class="px-4 py-3.5 text-sm font-semibold hover:bg-brand-red flex items-center space-x-1 focus:outline-none transition-all duration-150 {{ isset($district) ? 'bg-brand-red text-white' : '' }}">
                    ...
                    <div class="absolute left-0 mt-0 bg-white text-gray-800 shadow-xl rounded-b-md border border-gray-150 w-52 py-2 hidden group-hover:block hover:block z-40">
                        @if(isset($navDistricts))
                            @foreach($navDistricts as $dist)
                                <a href="{{ route('news.district', $dist->slug) }}" class="block px-4 py-2 text-sm font-medium hover:bg-gray-100 hover:text-brand-red transition-colors duration-150">
                                    {{ $dist->name }} बुलेटिन
                                </a>
                            @endforeach
                        @endif
            ...
            <!-- Glowing Live TV Direct Link -->
            <div class="hidden lg:flex items-center space-x-2 pr-4 shrink-0">
                <a href="#" onclick="event.preventDefault(); alert('लाइव वीडियो ब्रॉडकास्ट लोड हो रहा है... (मॉक)');" class="bg-brand-red hover:bg-brand-red-dark text-white font-bold px-3 py-1 rounded text-xs transition duration-150 animate-pulse-live flex items-center space-x-1">
                    <span class="w-1.5 h-1.5 bg-white rounded-full inline-block"></span>
                    <span>लाइव टीवी देखें</span>
                </a>
            </div>
        </div>
    </nav>
    ...
    <footer class="bg-brand-dark text-white border-t-4 border-brand-red py-10 px-4 mt-12">
        <div class="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div class="space-y-4">
                <h3 class="font-display font-bold text-lg border-b border-brand-dark-deep pb-2 text-brand-red uppercase tracking-wider">हमारे बारे में</h3>
                <p class="text-xs text-gray-400 leading-loose">
                    <strong>छत्तीसगढ़ न्यूज़ एक्सप्रेस (CG News Express)</strong> राज्य का प्रमुख डिजिटल न्यूज़ नेटवर्क है। हमारा उद्देश्य पाठकों तक निष्पक्षता, सत्यता और तीव्रता के साथ समाचार पहुंचाना है।
                </p>
            ...
            <div class="space-y-4">
                <h3 class="font-display font-bold text-lg border-b border-brand-dark-deep pb-2 text-brand-red uppercase tracking-wider">महत्वपूर्ण लिंक्स</h3>
            ...
            <!-- Col 3: Categories -->
            <div class="space-y-4">
                <h3 class="font-display font-bold text-lg border-b border-brand-dark-deep pb-2 text-brand-red uppercase tracking-wider">श्रेणियां</h3>
                <ul class="text-xs space-y-2 text-gray-400">
                    @if(isset($navCategories))
                        @foreach($navCategories as $cat)
                            <li><a href="{{ route('news.category', $cat->slug) }}" class="hover:underline hover:text-brand-red">{{ $cat->name }}</a></li>
                        @endforeach
                    @endif
                </ul>
            </div>
            ...
            <div class="space-y-4">
                <h3 class="font-display font-bold text-lg border-b border-brand-dark-deep pb-2 text-brand-red uppercase tracking-wider">सोशल मीडिया</h3>
                ...
                <div class="flex items-center space-x-2 pt-1">
                    <a href="#" onclick="event.preventDefault(); alert('फेसबुक पेज लोड हो रहा है...');" class="bg-brand-dark-deep hover:bg-brand-red px-3 py-1.5 rounded text-[10px] text-gray-300 font-bold border border-brand-dark-deep transition">Facebook</a>
                    <a href="#" onclick="event.preventDefault(); alert('ट्विटर फीड लोड हो रहा है...');" class="bg-brand-dark-deep hover:bg-brand-red px-3 py-1.5 rounded text-[10px] text-gray-300 font-bold border border-brand-dark-deep transition">X / Twitter</a>
                    <a href="#" onclick="event.preventDefault(); alert('यूट्यूब चैनल लोड हो रहा है...');" class="bg-brand-dark-deep hover:bg-brand-red px-3 py-1.5 rounded text-[10px] text-gray-300 font-bold border border-brand-dark-deep transition">YouTube</a>
                </div>
            </div>
        </div>
