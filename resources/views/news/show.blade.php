...
<style>
    @media print {
        ...
        header, footer, nav, aside, .accessibility-bar, .no-print, #hindi-clock, #hindi-date, .animate-ticker {
...
<div class="space-y-6">
    ...
    <nav class="text-xs text-gray-400 flex items-center space-x-1.5 no-print">
        ...
        <span>&rsaquo;</span>
        <a href="{{ route('news.category', $article->category->slug) }}" class="hover:underline hover:text-brand-red">{{ $article->category->name }}</a>
        <span>&rsaquo;</span>
        @if($article->district)
            <a href="{{ route('news.district', $article->district->slug) }}" class="hover:underline hover:text-brand-red">{{ $article->district->name }}</a>
            <span>&rsaquo;</span>
        @endif
        ...
        <div class="lg:col-span-8 space-y-6">
            ...
            <div class="bg-white p-6 md:p-8 rounded-xl border border-gray-200 shadow-2xs print-container relative">
                ...
                <div class="absolute -left-14 top-20 hidden xl:flex flex-col space-y-3 no-print">
                    <button onclick="shareOnWhatsApp()" class="w-10 h-10 rounded-full bg-green-500 hover:bg-green-600 text-white flex items-center justify-center shadow hover:scale-110 transition duration-150" title="व्हाट्सएप पर साझा करें">
                        <svg class="w-5 h-5 fill-current" viewBox="0 0 24 24">
                            <path d="M12 2C6.477 2 2 6.477 2 12c0 2.136.67 4.116 1.81 5.74L2.05 22l4.385-1.15C8.016 21.572 9.94 22 12 22c5.523 0 10-4.477 10-10S17.523 2 12 2zm0 1.62c4.628 0 8.38 3.753 8.38 8.38 0 4.628-3.753 8.38-8.38 8.38-1.892 0-3.645-.632-5.06-1.7l-.36-.214-2.585.678.69-2.523-.235-.373c-1.15-1.823-1.81-3.6-1.81-5.632 0-4.628 3.753-8.38 8.38-8.38z"/>
                        </svg>
                    </button>
                    <button onclick="copyArticleLink()" class="w-10 h-10 rounded-full bg-gray-500 hover:bg-gray-600 text-white flex items-center justify-center shadow hover:scale-110 transition duration-150" title="लिंक कॉपी करें">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" />
                        </svg>
                    </button>
                    <button onclick="window.print()" class="w-10 h-10 rounded-full bg-brand-dark hover:bg-brand-red text-white flex items-center justify-center shadow hover:scale-110 transition duration-150" title="प्रिंट करें">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="bg-gray-50 p-3 rounded-lg border border-gray-150 flex flex-wrap justify-between items-center my-4 gap-2 no-print text-xs text-gray-600">
                    <div class="flex items-center space-x-1.5">
                        <span>अक्षर आकार:</span>
                        <button onclick="localTextScale('small')" class="bg-white hover:bg-brand-red hover:text-white px-2 py-0.5 rounded border border-gray-250 font-medium">छोटा</button>
                        <button onclick="localTextScale('normal')" class="bg-white hover:bg-brand-red hover:text-white px-2 py-0.5 rounded border border-gray-250 font-medium bg-brand-red/5 border-brand-red/20 text-brand-red">सामान्य</button>
                        <button onclick="localTextScale('large')" class="bg-white hover:bg-brand-red hover:text-white px-2 py-0.5 rounded border border-gray-250 font-medium">बड़ा</button>
                    </div>
                    
                    <div class="flex items-center space-x-3">
                        <button onclick="window.print()" class="text-brand-dark hover:text-brand-red font-bold flex items-center space-x-1">
                ...
                <div id="article-detail-body" class="text-base leading-relaxed text-gray-800 space-y-4 transition-all duration-200">
                ...
                <div class="mt-8 pt-4 border-t border-gray-100 flex flex-wrap gap-2 items-center no-print">
                    ...
                    @foreach($article->tags as $tag)
                        <a href="{{ route('news.search') }}?query={{ $tag->name }}" class="text-xs bg-gray-100 hover:bg-brand-red hover:text-white px-2.5 py-1 rounded transition duration-100">
                            #{{ $tag->name }}
                        </a>
                    @endforeach
                </div>
            ...
            <section class="space-y-4 no-print">
                <h3 class="text-lg font-bold text-brand-dark flex items-center space-x-2 border-b-2 border-brand-red pb-1.5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="bg-white p-4 rounded-lg border border-gray-200 shadow-2xs hover:shadow-xs transition duration-150 flex flex-col justify-between">
                        <div>
                            <span class="text-[10px] font-bold text-brand-accent block uppercase">{{ $rel->category->name }}</span>
                            <h4 class="text-xs md:text-sm font-bold text-gray-800 line-clamp-2 hover:text-brand-red mt-1 leading-snug">
                                <a href="{{ route('news.show', $rel->slug) }}">{{ $rel->title }}</a>
                            </h4>
                        </div>
                        <a href="{{ route('news.show', $rel->slug) }}" class="text-xs font-bold text-brand-red hover:underline block mt-4 text-right">पढ़ें &rarr;</a>
                    </div>
                    @endforeach
                </div>
            </section>
            ...
            <section class="bg-white p-6 rounded-xl border border-gray-200 shadow-2xs space-y-4 no-print">
                ...
                <form id="comment-form" onsubmit="addMockComment(event)" class="space-y-3">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <input 
                            type="text" 
                            id="commenter-location" 
                            placeholder="स्थान (उदा. बिलासपुर)..." 
                            class="px-3 py-2 border border-gray-300 rounded text-xs focus:outline-none focus:ring-1 focus:ring-brand-red w-full bg-white"
                            required
                        >
                    </div>
                    <textarea 
                        id="comment-text" 
                        rows="3" 
                        placeholder="खबर पर अपनी राय व्यक्त करें..." 
                        ...
                        required
                    ></textarea>
                    <button type="submit" class="bg-brand-red hover:bg-brand-red-dark text-white px-4 py-2 rounded text-xs font-bold transition duration-150 border border-brand-red">
                        टिप्पणी सबमिट करें
                    </button>
                </form>
                ...
                <div id="comments-list" class="space-y-4 pt-2">
                    <div class="bg-gray-50 p-4 rounded border border-gray-100 text-xs space-y-1">
                        ...
                        <p class="text-gray-600 leading-relaxed">स्थानीय स्तर पर इस प्रकार के विकास और रोजगारोन्मुखी पहलों से लोगों को निश्चित रूप से फायदा मिल रहा है। इसे राज्य के अन्य जिलों में भी फैलाना चाहिए।</p>
                    </div>
                </div>
            </section>
