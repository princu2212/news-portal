...
<div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
    ...
    <div class="lg:col-span-8 space-y-10">
        ...
        <section class="space-y-4">
            <h2 class="text-xl font-bold font-display text-brand-dark flex items-center space-x-2 border-b-2 border-brand-red pb-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                <div class="md:col-span-7 bg-white rounded-lg overflow-hidden border border-gray-200 shadow-sm hover:shadow-md transition duration-200 group flex flex-col justify-between">
                    <a href="{{ route('news.show', $mainHero->slug) }}" class="block relative overflow-hidden h-64 md:h-80">
                        <img 
                            src="{{ $mainHero->image_url }}" 
                            alt="{{ $mainHero->title }}" 
                            class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                        >
                        <div class="absolute top-3 left-3 bg-brand-red text-white font-bold text-[10px] uppercase px-2 py-0.5 rounded tracking-wider shadow-sm flex items-center space-x-1">
                    ...
                    <div class="p-5 flex-1 flex flex-col justify-between space-y-3">
                        <div>
                            <span class="text-[10px] font-bold text-brand-red uppercase tracking-wider bg-brand-red/5 px-2 py-0.5 rounded">{{ $mainHero->category->name }}</span>
                ...
                <div class="md:col-span-5 flex flex-col gap-3">
                    ...
                    <div class="bg-white p-3 rounded-lg border border-gray-100 shadow-2xs hover:shadow-xs hover:border-gray-200 transition duration-150 flex space-x-3 items-center">
                        <a href="{{ route('news.show', $secHero->slug) }}" class="w-24 h-24 shrink-0 rounded overflow-hidden relative">
                            <img src="{{ $secHero->image_url }}" alt="{{ $secHero->title }}" class="w-full h-full object-cover">
                        </a>
                        <div class="flex-1 min-w-0">
                            <span class="text-[9px] font-bold text-brand-accent uppercase tracking-wider">{{ $secHero->category->name }}</span>
                            <h4 class="text-xs md:text-sm font-bold text-gray-800 line-clamp-2 leading-snug hover:text-brand-red mt-0.5">
                                <a href="{{ route('news.show', $secHero->slug) }}">{{ $secHero->title }}</a>
                            </h4>
                            <span class="text-[10px] text-gray-400 block mt-1">{{ $secHero->published_at->format('d M Y') }}</span>
                        </div>
                    </div>
                    @endforeach
                </div>
        ...
        <section class="space-y-4">
            <div class="flex justify-between items-end border-b-2 border-brand-red pb-2">
                ...
                <a href="{{ route('news.category', 'politics') }}" class="text-xs font-bold text-brand-red hover:underline">और देखें &rarr;</a>
            </div>
                ...
                <article class="bg-white rounded-lg border border-gray-200 shadow-2xs hover:shadow-sm hover:border-brand-red/20 transition duration-150 flex flex-col justify-between">
                    <div class="p-5 space-y-3">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-[10px] text-gray-400">{{ $item->district ? $item->district->name . ' बुलेटिन' : 'CG एक्सप्रेस न्यूज़' }}</span>
                        </div>
                    ...
                    <div class="px-5 py-3 bg-gray-50 border-t border-gray-100 flex justify-between items-center rounded-b-lg">
                        ...
                        <a href="{{ route('news.show', $item->slug) }}" class="text-xs font-bold text-brand-red hover:underline flex items-center space-x-0.5">
                            <span>पढ़ें खबर</span>
                            <span>&rarr;</span>
                        </a>
                    </div>
        ...
        <section class="bg-brand-red/5 p-6 rounded-xl border border-brand-red/10 space-y-4">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-2 border-b border-brand-red/20 pb-3">
                <div>
                ...
                <div class="flex flex-wrap gap-1">
                    @if(isset($navDistricts))
                        @foreach($navDistricts->take(5) as $idx => $dist)
                            <button 
                                onclick="switchDistrictTab('dist-{{ $dist->id }}', this)" 
            ...
            <div class="relative">
                @if(isset($navDistricts))
                    @foreach($navDistricts->take(5) as $idx => $dist)
                        ...
                        @endphp
                        <div id="dist-{{ $dist->id }}" class="district-news-panel {{ $idx == 0 ? 'block' : 'hidden' }} space-y-3">
                            @if($dNews->count() > 0)
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    @foreach($dNews as $dn)
                                    <div class="bg-white p-4 rounded-lg border border-gray-100 shadow-2xs hover:shadow-xs transition flex flex-col justify-between">
                                        <div>
                                            <span class="text-[10px] text-gray-400 font-bold block">{{ $dn->published_at->format('d M Y') }}</span>
                                            <h4 class="text-sm font-bold text-gray-800 line-clamp-2 hover:text-brand-red mt-1">
                                                <a href="{{ route('news.show', $dn->slug) }}">{{ $dn->title }}</a>
                                            </h4>
                                            <p class="text-xs text-gray-500 line-clamp-2 mt-2 leading-relaxed">{{ $dn->summary }}</p>
                                        </div>
                                        <a href="{{ route('news.show', $dn->slug) }}" class="text-xs font-bold text-brand-red hover:underline block mt-4 text-right">पढ़ें खबर &rarr;</a>
                                    </div>
                                    @endforeach
                                </div>
                                <div class="text-right pt-2">
                                    <a href="{{ route('news.district', $dist->slug) }}" class="text-xs font-bold text-brand-red hover:underline">
        ...
        <section class="space-y-4">
            <div class="flex justify-between items-end border-b-2 border-brand-red pb-2">
                ...
                <a href="{{ route('news.category', 'state') }}" class="text-xs font-bold text-brand-red hover:underline">सभी ख़बरें &rarr;</a>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @foreach($stateNews as $item)
                <div class="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-2xs hover:shadow-sm hover:border-brand-red/30 transition duration-150 flex flex-col justify-between">
                    <div class="h-44 overflow-hidden relative">
                        <img src="{{ $item->image_url }}" alt="{{ $item->title }}" class="w-full h-full object-cover">
                        <div class="absolute top-3 left-3 bg-brand-red text-white font-extrabold text-[10px] px-2.5 py-0.5 rounded tracking-wider shadow">
                            राज्य विशेष
                        </div>
                    </div>
                    <div class="p-5 flex-1 flex flex-col justify-between space-y-3">
                        ...
                        <div class="flex justify-between items-center text-xs text-gray-400 pt-3 border-t border-gray-50">
                            <span>{{ $item->published_at->format('d M Y') }}</span>
                            <a href="{{ route('news.show', $item->slug) }}" class="font-bold text-brand-red hover:underline">पूरी खबर पढ़ें &rarr;</a>
                        </div>
                    </div>
                </div>
                @endforeach
        ...
        <section class="space-y-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="relative overflow-hidden rounded-lg group h-40 border border-gray-200 shadow-2xs">
                    ...
                    <div class="absolute inset-0 bg-gradient-to-t from-black/85 via-black/40 to-transparent p-3 flex flex-col justify-end">
    ...
    <aside class="lg:col-span-4 space-y-8">
        ...
        <div id="opinion-poll-card" class="bg-gradient-to-b from-brand-dark to-brand-dark-deep text-white rounded-xl shadow-lg border border-brand-dark-deep overflow-hidden">
            <div class="bg-brand-red text-white text-xs font-bold uppercase py-2.5 px-4 flex justify-between items-center">
            ...
            <div id="poll-active-state" class="p-6 space-y-4">
                <h3 class="font-display font-bold text-sm text-gray-100 leading-relaxed">
                    प्रश्न: क्या छत्तीसगढ़ में पर्यटन के विकास से ग्रामीण क्षेत्रों के स्थानीय युवाओं को रोजगार के बेहतर अवसर मिल रहे हैं?
                </h3>
                
                <div class="space-y-2 pt-2">
                    <button onclick="submitMockVote('yes')" class="w-full text-left bg-brand-dark-deep hover:bg-brand-red text-xs py-3 px-4 rounded border border-gray-800 transition duration-150">
                        १. हाँ, काफी लाभ हो रहा है
                    </button>
                    <button onclick="submitMockVote('no')" class="w-full text-left bg-brand-dark-deep hover:bg-brand-red text-xs py-3 px-4 rounded border border-gray-800 transition duration-150">
                        २. नहीं, कोई खास असर नहीं है
                    </button>
                    <button onclick="submitMockVote('maybe')" class="w-full text-left bg-brand-dark-deep hover:bg-brand-red text-xs py-3 px-4 rounded border border-gray-800 transition duration-150">
            ...
            <div id="poll-result-state" class="p-6 space-y-5 hidden">
                <h3 class="font-display font-bold text-xs text-brand-accent uppercase tracking-wider">पोल के परिणाम (लाइव):</h3>
                ...
                <div class="bg-brand-dark-deep/50 p-3 rounded text-[10px] text-center text-gray-400 border border-gray-850">
        ...
        <div class="bg-brand-red/5 p-6 rounded-xl border border-brand-red/10 space-y-4">
            ...
            <form onsubmit="event.preventDefault(); alert('धन्यवाद! आपका नामांकन दर्ज कर लिया गया है।'); this.reset();" class="space-y-2">
                <input 
                    type="email" 
                    placeholder="अपना ईमेल पता यहाँ लिखें..." 
                    ...
                    required
                >
                <button type="submit" class="w-full bg-brand-red hover:bg-brand-red-dark text-white font-bold py-2 rounded text-xs transition duration-150 shadow-sm border border-brand-red">
        ...
        <div class="bg-white p-6 rounded-xl border border-gray-200 shadow-2xs space-y-3">
            ...
            <div class="flex flex-wrap gap-1.5">
                ...
                <a href="#" onclick="event.preventDefault(); alert('इस विषय पर समाचार खोजे जा रहे हैं...'); window.location.href='{{ route('news.search') }}?query={{ $tag->name }}'" class="text-xs bg-gray-100 hover:bg-brand-red hover:text-white px-2.5 py-1 rounded transition duration-150 font-medium">
                    #{{ $tag->name }}
                </a>
                @endforeach
            </div>
